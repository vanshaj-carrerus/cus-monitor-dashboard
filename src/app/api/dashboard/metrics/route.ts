...
async function resolveAllowedUsers(actor: Actor): Promise<AllowedUser[]> {
  const userFilter: Record<string, unknown> = { role: { $in: ["sales", "marketing"] } };
  ...
  if (actor.role === "sales" || actor.role === "marketing") {
    ...
    const [s, m] = (await Promise.all([
      SalesUser.find({ $or: or }).select("userId active").lean(),
      MarketingUser.find({ $or: or }).select("userId active").lean(),
    ])) as unknown as [ProfileRow[], ProfileRow[]];
    const allowedIds = [...new Set([...s, ...m].map((p) => p.userId.toString()))];
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [salesProfiles, marketingProfiles] = (await Promise.all([
      SalesUser.find({ userId: { $in: userIds } }).select("userId active").lean(),
      MarketingUser.find({ userId: { $in: userIds } }).select("userId active").lean(),
    ])) as unknown as [ProfileRow[], ProfileRow[]];
    const activeMap = new Map<string, boolean>();
    for (const p of [...salesProfiles, ...marketingProfiles]) {
